...
EVT_BUTTON(1004, Battle2::OnClickRetreat)
EVT_BUTTON(1005, Battle2::OnClickClaim)
EVT_BUTTON(1006, Battle2::OnClickBackMenu)
END_EVENT_TABLE()
...
*/
		...
		pdc.DrawBitmap(*rs->drop[2], wxPoint(122, 248));
...
void Battle2::OnClickRetreat(wxCommandEvent & event)
{
...
void Battle2::OnClickClaim(wxCommandEvent & event)
{
	wxMessageOutputDebug().Printf("claim");
		...
		mirai->itmwood += 2;
		mirai->bondhij += 1;
		wxMessageOutputDebug().Printf("Mirai brick %d earth %d", mirai->itmlog, mirai->itmbrick);
...
void Battle2::OnClickBackMenu(wxCommandEvent & event)
{
